...
var Omnitone = (function () {
  ...
  const otUtils = {};
  otUtils.log = function() {
    const message = `[Omnitone] \
${Array.prototype.slice.call(arguments).join(' ')} \
(${performance.now().toFixed(2)}ms)`;
    window.console.log(message);
  };
  otUtils.throw = function() {
    const message = `[Omnitone] \
${Array.prototype.slice.call(arguments).join(' ')} \
(${performance.now().toFixed(2)}ms)`;
  ...
  let b11;
  let det;
  otUtils.invertMatrix4 = function(out, a) {
    a00 = a[0];
    a01 = a[1];
    a02 = a[2];
    a03 = a[3];
  ...
  otUtils.isDefinedENUMEntry = function(enumDictionary, entryValue) {
  ...
  otUtils.isAudioContext = function(context) {
    return context instanceof AudioContext ||
      context instanceof OfflineAudioContext;
  };
  otUtils.isAudioBuffer = function(audioBuffer) {
    return audioBuffer instanceof AudioBuffer;
  };
  otUtils.mergeBufferListByChannel = function(context, bufferList) {
    ...
    for (let i = 0; i < bufferList.length; ++i) {
      if (bufferNumberOfChannel > 32) {
        otUtils.throw('Utils.mergeBuffer: Number of channels cannot exceed 32.' +
            '(got ' + bufferNumberOfChannel + ')');
      }
      if (bufferLength !== bufferList[i].length) {
        otUtils.throw('Utils.mergeBuffer: AudioBuffer lengths are ' +
            'inconsistent. (expected ' + bufferLength + ' but got ' +
            bufferList[i].length + ')');
      }
      if (bufferSampleRate !== bufferList[i].sampleRate) {
        otUtils.throw('Utils.mergeBuffer: AudioBuffer sample rates are ' +
  ...
  otUtils.splitBufferbyChannel = function(context, audioBuffer, splitBy) {
    if (audioBuffer.numberOfChannels <= splitBy) {
      otUtils.throw('Utils.splitBuffer: Insufficient number of channels. (' +
  ...
  otUtils.getArrayBufferFromBase64String = function(base64String) {
  ...
  function BufferList(context, bufferData, options) {
    this._context = otUtils.isAudioContext(context) ?
        context :
        otUtils.throw('BufferList: Invalid BaseAudioContext.');
    this._options = {
    ...
    if (options) {
      if (options.dataType &&
          otUtils.isDefinedENUMEntry(BufferDataType, options.dataType)) {
  ...
  BufferList.prototype._promiseGenerator = function(resolve, reject) {
    if (typeof resolve !== 'function') {
      otUtils.throw('BufferList: Invalid Promise resolver.');
    } else {
  ...
  BufferList.prototype._launchAsyncLoadTask = function(taskId) {
    ...
    this._context.decodeAudioData(
        otUtils.getArrayBufferFromBase64String(this._bufferData[taskId]),
  ...
  BufferList.prototype._updateProgress = function(taskId, audioBuffer) {
    ...
    if (this._options.verbose) {
      ...
      otUtils.log('BufferList: ' + messageString + ' successfully loaded.');
    }
    if (--this._numberOfTasks === 0) {
      ...
      otUtils.log('BufferList: ' + messageString + ' loaded successfully.');
  ...
  function FOARenderer(context, config) {
    this._context = otUtils.isAudioContext(context) ?
        context :
        otUtils.throw('FOARenderer: Invalid BaseAudioContext.');
    this._config = {
  ...
  FOARenderer.prototype._initializeCallback = function(resolve, reject) {
    ...
    bufferList.load().then(
        function(hrirBufferList) {
          ...
          otUtils.log('FOARenderer: HRIRs loaded successfully. Ready.');
          resolve();
        }.bind(this),
        function() {
          const errorMessage = 'FOARenderer: HRIR loading/decoding failed.';
          reject(errorMessage);
          otUtils.throw(errorMessage);
        });
  };
  ...
  FOARenderer.prototype.setRotationMatrixFromCamera = function(cameraMatrix) {
    ...
    otUtils.invertMatrix4(this._tempMatrix4, cameraMatrix.elements);
  ...
  FOARenderer.prototype.setRenderingMode = function(mode) {
    ...
    otUtils.log('FOARenderer: Rendering mode changed. (' + mode + ')');
  };
  ...
  function HOARenderer(context, config) {
    this._context = otUtils.isAudioContext(context) ?
        context :
        otUtils.throw('HOARenderer: Invalid BaseAudioContext.');
    this._config = {
      ambisonicOrder: 3,
  ...
  HOARenderer.prototype._initializeCallback = function(resolve, reject) {
    ...
    bufferList.load().then(
        function(hrirBufferList) {
          ...
          this.setRenderingMode(this._config.renderingMode);
          this._isRendererReady = true;
          otUtils.log('HOARenderer: HRIRs loaded successfully. Ready.');
          resolve();
        }.bind(this),
        function() {
  ...
  HOARenderer.prototype.setRenderingMode = function(mode) {
    ...
    this._config.renderingMode = mode;
    otUtils.log('HOARenderer: Rendering mode changed. (' + mode + ')');
  };
  ...
  Omnitone.mergeBufferListByChannel = otUtils.mergeBufferListByChannel;
  Omnitone.splitBufferbyChannel = otUtils.splitBufferbyChannel;
  ...
  (function() {
    otUtils.log(`Version ${Version} (running ${Omnitone.browserInfo.name} \
${Omnitone.browserInfo.version} on ${Omnitone.browserInfo.platform})`);
    if (Omnitone.browserInfo.name.toLowerCase() === 'safari') {
      ...
      otUtils.log(`${Omnitone.browserInfo.name} detected. Polyfill applied.`);
    }
  })();
